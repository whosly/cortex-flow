...
这是一个使用 CortexFlow 框架构建的数据分析管道示例项目，展示了框架的核心功能。

## 功能说明

Demo 实现了一个数据分析管道，包含以下处理步骤：
...
- **数据清洗**：去重、格式化、统计汇总
- **数据验证**：校验数据完整性和有效性
- **LLM 分析**：调用大语言模型进行智能分析
- **报告生成**：生成结构化分析报告
- **可视化导出**：导出 Mermaid 格式图表
...
DAG 结构:
  ...
  [generate_report] 报告已生成: 数据分析报告 - 2026-05-19 (无（基础模式）)
...
--- Demo 2: 带 LLM 分析的数据管道 ---
  ...
  [generate_report] 智能报告已生成: 数据分析报告 - 2026-05-19 (LLM 智能分析)
...
场景2: 重试耗尽（始终失败）
  ...
  第 2 次尝试: 失败
  第 3 次尝试: 失败
  重试结果: LLM call failed: API rate limit
...
Session ID: 35f36846-92da-45bc-bf9d-a0edd86e2e9d
设置上下文数据:
  pipeline_name = 数据分析管道
  config = Object {"max_retries": Number(3), "threshold": Number(0.8)}
  batch_size = 100
...
--- Demo 5: 带进度回调的管道 ---
  ...
  [fetch] 完成
  进度: [████░░░░░░░░░░░░░░░░░░░░░░░░░░] 16% (1/6 成功:1 失败:0)  [validate] 完成
  [clean] 完成
  进度: [███████████████░░░░░░░░░░░░░░░] 50% (3/6 成功:3 失败:0)  [analyze] 完成
  进度: [███████████████████░░░░░░░░░░░] 66% (4/6 成功:4 失败:0)  [export] 完成
  [report] 完成
...
默认配置 model: gpt-4
ChatRequest 覆盖: model=gpt-3.5-turbo, temperature=Some(0.3), max_tokens=Some(512)

=== 3. Orchestrator Builder 多模型注册 ===

已注册模型: ["gpt4", "mock", "doubao", "llama3"]
默认模型: Some("gpt4")
获取 gpt4: true
获取 doubao: true
...
DAG 结构（任务通过 ctx.get_llm() 获取模型）:
  ...
  [gpt4_analysis] 使用 GPT-4 分析 (10 条记录)...
  [llama3_validate] 使用本地 Llama3 校验 (10 条)...
  [statistics] 计算统计指标...
  ...
  [token_summary] Token 使用统计:
    ...
    总 Token: 0
    估算成本: $0.0000
  [token_summary] 可用模型: ["gpt4", "mock", "doubao", "llama3"]
...
节点结果:
  [OK] fetch_data (获取销售数据) - 0ms
  [OK] clean_data (清洗数据) - 0ms
  [OK] gpt4_analysis (GPT-4 深度分析) - 0ms
  [OK] llama3_validate (Llama3 本地校验) - 0ms
  [OK] statistics (统计计算) - 0ms
  [OK] doubao_report (豆包报告润色) - 0ms
  [OK] token_summary (Token 使用统计) - 0ms
...
注册表中的模型: ["llama3", "gpt4", "doubao", "mock"]
默认模型: Some("gpt4")
包含 gpt4: true
模型数量: 4
